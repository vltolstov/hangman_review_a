...
а) Рендерер висельницы- картинки и метод распечатки  
б) Собственно хангман, человек, которого вешают- счетчик ошибок(стадий повешения) и метод увеличения стадии повешения  
в) Отдельная сущность Определитель результатов игры(либо ответственность сущности Игра в части определения конца игры)- метод определения конца игры
...
+ (+)Делает только то, что должен- ведет диалог с пользователем и по его результатам создает и запускает Игру(GameSession), это хорошо
...
Задание не выполнено в полном объеме, есть серьезные баги. Проект нужно дорабатывать. 
