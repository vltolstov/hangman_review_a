...
**2. Нарушение DRY**, классический пример последствий дублирования кода. Рассинхронизация текста: в одном месте с точкой, в другом без
```
...
**?. Если в блоке if есть return**(break, continue, throw, exit и т.д.), то else не пишется - в этом случае неважно, будет else или нет, так как программа будет работать одинаково
```
