...
2. Порядок отображения ошибочно введенных букв неправильный: не по алфовиту и не по последовательности ввода
```
...
Здесь же все не так и класс Клавиатура хранит буквы алфавита. Почему бы тогда не назвать класс "Алфавитом"?
...
```
Логика интересная, я бы сказал, что она обратная привычному походу для хангмана.
...
Здесь: изнаачльно существует перечень всех букв, при отгадывании(вводе) они удаляются из этого перечня.
...
Тем самым, есть неявные правила для корректного использования этого класса- проверяя буквы на нажатость мы должны передавать в него буквы, которые заведомо входили в состав перечня букв на момент создания класса через конструктор по умоляанию.
Подобные допущения и неявные правила для корректного пользования классами- прямой путь к глюкам и багам.

В данном случае, чтобы класс работал корректно, он должен хранить и исходный перечень букв(алфавит) и текущий перечень букв(ненажатые буквы).  
И учитывать исходный перечень при проверке буквы. Примерно так
```
public boolean isLetterTapped(char letter) {
...
for (int row = 0; row < picture.length; row++) {
  System.out.println([rowInOneState]);
}
```
...
Здесь при открывании буквы объект пересоздается с новыми данными, то есть в том числе с открытой буквой
```
final RightLettersInWord rightLettersInWordAfterTap = rightLettersInWord.performOpenLetter(letter);
```
И так со всеми классами.

На мой взгляд, такая приницпиальная иммутабельность делает этот проект более сложным, операции избыточными, а выгоды мне не очевидны.  
