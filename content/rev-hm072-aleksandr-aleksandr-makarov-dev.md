...
- В связи с вышесказанным, Game должен принимать текст слова для отгадывания в конструктор. Класс не должен знать, откуда взялось это слово- из файла, по сети, по интерфейсу RS-485 etc.
...
Этот диалог должен происходить выше, в данном случае- в классе Main, либо в каком-то Стартере.
...
- ООП должен упрощать код, а здесь он его усложняет. При этом ООП здесь довольно спорадический.  
...
- Если в программе скрытые буквы интерпритируются как отдельная сущность и вынесены в класс, то также следует сделать класс Скрытое слово. 
...
Здесь наоборот процесс игры от начала и до конца происходит одинаково- ввод букв, пока игра не закончится тем или иным результатом.
